 ...
 */
const extractNameFromTemplate = (value) => value.split('Hello, ').join('').split('!').join('');
  ...
  for (let i = 0; i < height; i += 1) {
    for (let j = 0; j < width; j += 1) {
      if (i === 0 && j === 0) {
        ...
        result.push(RIGHT_DONW);
      } else if (i === 0 || i === height - 1) {
        result.push(HORISONTAL);
 ...
 *
 */
const DATA = ['ABCDEFGHIJKLMabcdefghijklm'.split(''), 'NOPQRSTUVWXYZnopqrstuvwxyz'.split('')];
function encodeToRot13(str) {
  return str.split('').map((ch) => {
    if (DATA[0].includes(ch)) return DATA[1][DATA[0].indexOf(ch)];
    if (DATA[1].includes(ch)) return DATA[0][DATA[1].indexOf(ch)];
    return ch;
  }).join('');
}
...
const getCardId = (value) => CARD_IDS.indexOf(value);
