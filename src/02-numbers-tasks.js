...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const width = Math.abs(x1) + Math.abs(x2);
  const height = Math.abs(y1) + Math.abs(y2);
...
const getAngleBetweenVectors = (x1, y1, x2, y2) => {
  const num = (x1 * x2 + y1 * y2);
  const den = (Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2));
 ...
 *     0     => 0
 */
const getLastDigit = (value) => value.toString().split('').pop();
